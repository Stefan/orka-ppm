      ...
      if (isActualButton && !hasHoverShadow && light.bgColor === '#ffffff' && light.borderColor) {
        ...
        if (borderVsWhite < 1.5) {
          violations.push({
            ...
            ratio: Math.round(borderVsWhite * 100) / 100,
            required: 1.5,
...
describe('UI Contrast Compliance (WCAG AA)', () => {
  ...
  describe('Light Mode — explicit text+bg on same element', () => {
    ...
    beforeAll(() => {
      for (const file of tsxFiles) {
        lightViolations.push(
          ...scanFileForExplicitViolations(file).filter(v => v.mode === 'light')
        );
      }
    });
  ...
  describe('Specific high-risk areas', () => {
    ...
    it('Quick Actions bar: text should be at least gray-800 for readability', () => {
      ...
      const hasGoodText =
        qaSection.includes('text-gray-800') || qaSection.includes('text-gray-900');
    ...
    it('TopBar: dropdown items should have visible hover effects', () => {
      ...
      // [\s\S]* matches newlines (variable def may span lines)
      expect(content).toMatch(/dropdownItemInactive[\s\S]*?hover:bg-/);
      expect(content).toMatch(/navLinkInactive[\s\S]*?hover:bg-/);
    });
    ...
    it('critical.css should not have unlayered Tailwind class definitions', () => {
      ...
      expect(unlayeredClassDefs).not.toMatch(
        /\.(flex|grid|text-gray|bg-white|rounded|shadow|border-gray)\s*\{/
      );
    });
    ...
    it('globals.css and critical.css: all element resets must be inside @layer base', () => {
      ...
      for (const file of filesToCheck) {
        ...
        const hasUnlayeredRules =
          /\b(html|body|button|a|input|select|textarea)\s*\{[^}]*\b(color|background|border|font)\b/i.test(
            result
          );
