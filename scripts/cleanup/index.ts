...
import { FileCategory, CleanupStats, ArchiveResult, DeletionResult, FileInfo } from './types';
 ...
 */
function log(message: string, verbose: boolean, force: boolean = false): void {
  if (force || verbose) {
    console.log(message);
  }
}
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const sqlChecker = new SqlReferenceChecker(rootDir);
    ...
    const allArchiveResults: ArchiveResult[] = [];
    const allDeletionResults: DeletionResult[] = [];
    ...
    // Flag unknown files
    const unknownFiles = categorized.get(FileCategory.UNKNOWN) || [];
    ...
    const preservedFiles = categorized.get(FileCategory.ESSENTIAL) || [];
    ...
    const deletedByCategory = new Map<FileCategory, FileInfo[]>();
    const archivedByCategory = new Map<FileCategory, ArchiveResult[]>();
    ...
    const reportContent = reportGenerator.generateSummary(
      stats,
      deletedByCategory,
      archivedByCategory,
      preservedFiles,
    ...
    const gitignoreUpdater = new GitignoreUpdater(rootDir);
