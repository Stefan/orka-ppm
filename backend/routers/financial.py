...
async def get_financial_tracking_budget_alerts(
    ...
    try:
        ...
        # Get all projects with budget information (may fail if table/RLS not set up)
        try:
            projects_response = supabase.table("projects").select("id, name, budget").execute()
            projects = projects_response.data or []
        except Exception as proj_err:
            logger.warning("Budget alerts: projects query failed (%s), returning empty alerts", proj_err)
            return {
        ...
        # Single query for all spending (avoids N+1); financial_tracking table may not exist
        try:
            ...
            logger.warning("Budget alerts: financial_tracking query failed (%s), using no expenses", ft_err)
            expenses = []
