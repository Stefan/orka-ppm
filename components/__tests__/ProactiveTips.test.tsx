...
describe('ProactiveTips Component', () => {
  ...
  describe('Accessibility Features', () => {
      ...
      const tipMessages: ChatMessage[] = [
        {
          id: 'tip-1',
          type: 'tip',
          content: '💡 **Accessibility Tip**\n\nThis tip tests accessibility features.',
          timestamp: new Date(),
          actions: [
            {
              id: 'action-1',
    ...
    it('has proper ARIA structure', () => {
      ...
      // Main region (get the first one which is the container)
      const regions = screen.getAllByRole('region')
      const mainRegion = regions[0]
      expect(mainRegion).toHaveAttribute('aria-labelledby', 'tips-container-title')
      expect(mainRegion).toHaveAttribute('aria-describedby', 'tips-container-description')
    ...
    it('provides screen reader announcements', () => {
      ...
      // Check for live regions (there are multiple, get the first one)
      const announceRegions = document.querySelectorAll('[aria-live="polite"]')
      expect(announceRegions.length).toBeGreaterThan(0)
      
      const firstAnnounceRegion = announceRegions[0]
      expect(firstAnnounceRegion).toHaveAttribute('aria-live', 'polite')
      expect(firstAnnounceRegion).toHaveAttribute('aria-atomic', 'true')
    })
    ...
    it('has proper keyboard navigation', async () => {
      ...
      // Get the main container region (first one)
      const regions = screen.getAllByRole('region')
      const mainRegion = regions[0]
      expect(mainRegion).toBeInTheDocument()
      
      // Should handle basic keyboard interaction
      expect(mainRegion).toHaveAttribute('role', 'region')
    })
    ...
    it('provides proper button accessibility', () => {
      ...
      expect(minimizeButton).toHaveAttribute('title')
      ...
      expect(actionButton).toHaveAttribute('aria-label', 'Test Action')
      expect(actionButton).toHaveClass('min-h-[32px]') // Touch target size
    })

    it('provides proper time information', () => {
      render(<ProactiveTips />)
