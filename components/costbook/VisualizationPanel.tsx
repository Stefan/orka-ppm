...
function EVMTrendChartSection({
  ...
  if (evmHistory.length === 0) {
    return (
      <div className="flex items-center justify-center h-[140px] text-gray-500 dark:text-slate-400 text-sm">
    ...
    <div className={`space-y-4 ${className}`}>
      ...
      <div className="grid grid-cols-2 gap-4 text-center">
        <div className="p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
          ...
          <div className="text-xs text-blue-600 dark:text-blue-400/70">Projects</div>
        </div>
        <div className="p-3 bg-green-50 dark:bg-green-900/20 rounded-lg">
          <div className="text-2xl font-bold text-green-600 dark:text-green-400">{kpis.under_budget_count}</div>
          <div className="text-xs text-green-600 dark:text-green-400/70">Under Budget</div>
        </div>
        <div className="p-3 bg-red-50 dark:bg-red-900/20 rounded-lg">
          <div className="text-2xl font-bold text-red-600 dark:text-red-400">{kpis.over_budget_count}</div>
          <div className="text-xs text-red-600 dark:text-red-400/70">Over Budget</div>
        </div>
        <div className={`p-3 rounded-lg ${kpis.net_variance >= 0 ? 'bg-green-50 dark:bg-green-900/20' : 'bg-red-50 dark:bg-red-900/20'}`}>
          <div className={`text-2xl font-bold ${kpis.net_variance >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
            {Math.round(Math.abs(kpis.net_variance) / 1000)}K
          </div>
          <div className={`text-xs ${kpis.net_variance >= 0 ? 'text-green-600 dark:text-green-400/70' : 'text-red-600 dark:text-red-400/70'}`}>
