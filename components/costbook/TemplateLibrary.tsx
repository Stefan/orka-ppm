...
export function TemplateLibrary({
  ...
  return (
    <div className={`bg-white dark:bg-slate-800 rounded-lg shadow border border-gray-200 dark:border-slate-700 p-4 ${className}`}>
      <div className="flex justify-between items-center mb-4">
        <h3 className="font-semibold text-gray-900 dark:text-slate-100">Import Templates</h3>
        {onClose && (
          <button type="button" onClick={onClose} className="text-gray-500 hover:text-gray-700 dark:hover:text-slate-300 dark:text-slate-300 text-sm">
            Close
          </button>
        )}
      </div>
      <label className="flex items-center gap-2 text-sm text-gray-600 dark:text-slate-400 mb-3">
        <input
          type="checkbox"
      ...
      {loading ? (
        ...
        <p className="text-sm text-gray-500 dark:text-slate-400 py-4">No templates saved yet.</p>
      ) : (
        <ul className="space-y-2">
          {templates.map((t) => (
            <li
              key={t.id}
              className="flex items-center justify-between gap-2 p-2 rounded bg-gray-50 dark:bg-slate-800/50 hover:bg-gray-100 dark:hover:bg-slate-600"
            >
              ...
              <div className="flex items-center gap-1 flex-shrink-0">
                <button
                  ...
                  className="text-xs px-2 py-1 rounded bg-blue-100 dark:bg-blue-900/30 text-blue-700 hover:bg-blue-200"
                >
                  Load
                </button>
                <button
