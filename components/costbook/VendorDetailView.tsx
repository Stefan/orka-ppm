...
function MetricCard({
  ...
  return (
    <div className="bg-gray-50 dark:bg-slate-800/50 rounded-lg p-3">
      ...
      {subValue && (
        <div className="text-xs text-gray-400 dark:text-slate-500">{subValue}</div>
      )}
    </div>
  )
}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      `}>
        ...
        <div className="flex items-start justify-between p-6 border-b border-gray-200 dark:border-slate-700">
          ...
          <button
            onClick={onClose}
            className="absolute top-4 right-4 p-2 text-gray-400 hover:text-gray-600 dark:text-slate-300 hover:bg-gray-100 dark:hover:bg-slate-600 dark:bg-slate-700 rounded-full transition-colors"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
            ...
            <button
              ...
              className={`
                ...
                ${activeTab === tab
                  ...
                  : 'text-gray-500 hover:text-gray-700 dark:hover:text-slate-300 dark:text-slate-300'
                }
              `}
            >
              {tab}
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                <MetricCard
                  ...
                  icon={Clock}
                  color={vendor.score.on_time_delivery_rate >= 90 ? 'text-green-600 dark:text-green-400' : 
                         vendor.score.on_time_delivery_rate >= 75 ? 'text-yellow-600 dark:text-yellow-400' : 'text-red-600 dark:text-red-400'}
                />
                <MetricCard
                  label="Cost Variance"
                  value={formatPercentage(vendor.score.cost_variance_percentage)}
                  icon={DollarSign}
                  color={Math.abs(vendor.score.cost_variance_percentage) <= 5 ? 'text-green-600 dark:text-green-400' : 
                         Math.abs(vendor.score.cost_variance_percentage) <= 10 ? 'text-yellow-600 dark:text-yellow-400' : 'text-red-600 dark:text-red-400'}
                />
                <MetricCard
                  label="Quality Score"
                  value={vendor.score.quality_score}
                  icon={Star}
                  color={vendor.score.quality_score >= 85 ? 'text-green-600 dark:text-green-400' : 
                         vendor.score.quality_score >= 70 ? 'text-yellow-600 dark:text-yellow-400' : 'text-red-600 dark:text-red-400'}
                />
                <MetricCard
                  label="Response Time"
                  value={vendor.score.response_time_score}
                  icon={BarChart3}
                  color={vendor.score.response_time_score >= 85 ? 'text-green-600 dark:text-green-400' : 
                         vendor.score.response_time_score >= 70 ? 'text-yellow-600 dark:text-yellow-400' : 'text-red-600 dark:text-red-400'}
                />
              </div>
              
              {/* Performance summary */}
              <div className="bg-gray-50 dark:bg-slate-800/50 rounded-lg p-4">
                <h3 className="font-medium text-gray-900 dark:text-slate-100 mb-3">Performance Summary</h3>
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    <span className="text-gray-500 dark:text-slate-400">Projects Completed:</span>
                  ...
                  <div>
                    <span className="text-gray-500 dark:text-slate-400">Deliveries (on-time/total):</span>
                    <span className="ml-2 font-medium">
                      {vendor.metrics.total_deliveries - vendor.metrics.late_deliveries}/{vendor.metrics.total_deliveries}
                    </span>
                  </div>
                  <div>
                    <span className="text-gray-500 dark:text-slate-400">Quality Issues:</span>
                    <span className="ml-2 font-medium">{vendor.metrics.quality_issues}</span>
                  </div>
                  <div>
                    <span className="text-gray-500 dark:text-slate-400">Avg Response Time:</span>
                    <span className="ml-2 font-medium">{vendor.metrics.avg_response_time_hours.toFixed(1)}h</span>
                  </div>
                  <div>
                    <span className="text-gray-500 dark:text-slate-400">Avg Delivery Days:</span>
              ...
              {(vendor.contact_email || vendor.contact_phone || vendor.address) && (
                <div className="bg-gray-50 dark:bg-slate-800/50 rounded-lg p-4">
                  <h3 className="font-medium text-gray-900 dark:text-slate-100 mb-3">Contact Information</h3>
                  <div className="space-y-2 text-sm">
                    {vendor.contact_email && (
                      <div className="flex items-center gap-2 text-gray-600 dark:text-slate-400">
                        <Mail className="w-4 h-4" />
                        <a href={`mailto:${vendor.contact_email}`} className="hover:text-blue-600 dark:text-blue-400">
                          {vendor.contact_email}
                        </a>
                      </div>
                    )}
                ...
                <>
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    <div className="bg-gray-50 dark:bg-slate-800/50 rounded-lg p-4 text-center">
                      ...
                      <div className="text-xs text-gray-500 dark:text-slate-400">12-Month Avg</div>
                    </div>
                    <div className="bg-gray-50 dark:bg-slate-800/50 rounded-lg p-4 text-center">
                      <div className={`text-2xl font-bold ${
                        history.year_over_year_change > 0 ? 'text-green-600 dark:text-green-400' : 
                        history.year_over_year_change < 0 ? 'text-red-600 dark:text-red-400' : 'text-gray-600 dark:text-slate-400'
                      }`}>
                        {history.year_over_year_change > 0 ? '+' : ''}{history.year_over_year_change.toFixed(1)}
                      </div>
                      <div className="text-xs text-gray-500 dark:text-slate-400">YoY Change</div>
                    </div>
                    <div className="bg-gray-50 dark:bg-slate-800/50 rounded-lg p-4 text-center">
                      <TrendIndicator trend={history.trend} />
                      <div className="text-xs text-gray-500 dark:text-slate-400 mt-1">Trend</div>
                    </div>
                  </div>
                  
                  {/* Performance chart */}
                  <div>
                    <h3 className="font-medium text-gray-900 dark:text-slate-100 mb-3">Performance Over Time</h3>
                    <div className="h-64">
                ...
                vendor.projects.map(project => (
                  ...
                  >
                    <div className="flex items-start justify-between mb-2">
                      <div>
                        <h4 className="font-medium text-gray-900 dark:text-slate-100">{project.project_name}</h4>
                    ...
                    <div className="grid grid-cols-3 gap-3 mt-3 text-sm">
                      ...
                      <div>
                        ...
                        <span className={`ml-1 font-medium ${
                          project.variance >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'
                        }`}>
        ...
        <div className="px-6 py-4 border-t border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-800/50">
          <div className="flex items-center justify-between text-xs text-gray-500 dark:text-slate-400">
            ...
            <button
              onClick={onClose}
              className="px-4 py-2 bg-gray-200 text-gray-700 dark:text-slate-300 rounded-lg hover:bg-gray-300 transition-colors text-sm"
            >
              Close
            </button>
          </div>
