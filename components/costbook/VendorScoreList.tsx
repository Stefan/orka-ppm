...
function StatusIndicator({ status }: { status: VendorStatus }) {
  const config: Record<VendorStatus, { icon: React.ReactNode; color: string; label: string }> = {
    active: { icon: <CheckCircle2 className="w-3 h-3" />, color: 'text-green-600 dark:text-green-400', label: 'Active' },
    inactive: { icon: <Clock className="w-3 h-3" />, color: 'text-gray-400 dark:text-slate-500', label: 'Inactive' },
    pending: { icon: <Clock className="w-3 h-3" />, color: 'text-yellow-600 dark:text-yellow-400', label: 'Pending' },
    suspended: { icon: <Ban className="w-3 h-3" />, color: 'text-red-600 dark:text-red-400', label: 'Suspended' }
  }
    ...
    <div className={`bg-white dark:bg-slate-800 rounded-lg ${className}`}>
      ...
      <div className="p-4 border-b border-gray-200 dark:border-slate-700">
        <div className="flex items-center justify-between mb-4">
          ...
          <button
            onClick={loadVendors}
            disabled={loading}
            className="p-1.5 text-gray-400 hover:text-gray-600 dark:text-slate-300 hover:bg-gray-100 dark:hover:bg-slate-600 dark:bg-slate-700 rounded-lg transition-colors"
            title="Refresh"
          >
        ...
        <div className="space-y-3">
          <div className="flex gap-2">
            ...
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full pl-9 pr-4 py-2 border border-gray-200 dark:border-slate-700 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              />
            </div>
            ...
            <button
              ...
              className={`
                ...
                ${showFilters || hasActiveFilters
                  ...
                  : 'border-gray-200 dark:border-slate-700 text-gray-600 dark:text-slate-400 hover:bg-gray-50 dark:hover:bg-slate-700 dark:bg-slate-800/50'
                }
              `}
            >
          ...
          {/* Filter panel */}
          {showFilters && (
            <div className="p-3 bg-gray-50 dark:bg-slate-800/50 rounded-lg border border-gray-200 dark:border-slate-700 space-y-3">
              {/* Rating filter */}
              <div>
                <label className="text-xs font-medium text-gray-500 dark:text-slate-400 mb-1 block">Rating</label>
                    ...
                    <button
                      ...
                      className={`
                        ...
                        ${(filter.rating || []).includes(rating)
                          ...
                          : 'bg-gray-100 dark:bg-slate-700 text-gray-400 dark:text-slate-300 hover:bg-gray-200 dark:hover:bg-slate-600'
                        }
                      `}
                    >
                      {rating}
              ...
              {/* Status filter */}
              <div>
                <label className="text-xs font-medium text-gray-500 dark:text-slate-400 mb-1 block">Status</label>
      ...
      <div className="px-4 py-2 border-b border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-800/50">
        <div className="flex items-center gap-4 text-xs">
          <span className="text-gray-500 dark:text-slate-400">Sort by:</span>
          {[
          ...
          <div className="divide-y divide-gray-100">
            {filteredVendors.map(vendor => (
              <button
                ...
                className={`
                  w-full p-4 text-left hover:bg-gray-50 dark:hover:bg-slate-700 dark:bg-slate-800/50 transition-colors
                  ${selectedVendorId === vendor.id ? 'bg-blue-50 dark:bg-blue-900/20' : ''}
                `}
              >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className={`text-lg font-bold ${
                        vendor.score.overall_score >= 75 ? 'text-green-600 dark:text-green-400' : 
                        vendor.score.overall_score >= 50 ? 'text-yellow-600 dark:text-yellow-400' : 'text-red-600 dark:text-red-400'
                      }`}>
                    ...
                    <div className="flex items-center gap-4 text-xs text-gray-500 dark:text-slate-400">
                      ...
                      <span className={vendor.score.cost_variance_percentage > 5 ? 'text-red-600 dark:text-red-400' : ''}>
