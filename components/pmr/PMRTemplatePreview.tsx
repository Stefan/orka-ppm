    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4">
      <div className="bg-white dark:bg-slate-800 rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="p-6 border-b border-gray-200 dark:border-slate-700">
          <div className="flex items-start justify-between">
            <div className="flex-1">
              <h2 className="text-2xl font-bold text-gray-900 dark:text-slate-100 mb-2">{template.name}</h2>
              <p className="text-sm text-gray-600 dark:text-slate-400">{template.description}</p>
              <div className="flex items-center space-x-3 mt-3">
                <span className="px-3 py-1 text-xs font-medium text-blue-700 bg-blue-100 dark:bg-blue-900/30 rounded-full capitalize">
                  {template.template_type}
                </span>
                {template.industry_focus && (
                  <span className="px-3 py-1 text-xs font-medium text-green-700 bg-green-100 dark:bg-green-900/30 rounded-full">
                    {template.industry_focus}
                  </span>
                )}
                {template.is_public && (
                  <span className="px-3 py-1 text-xs font-medium text-purple-700 bg-purple-100 dark:bg-purple-900/30 rounded-full">
                    Public Template
                  </span>
                )}
              </div>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {/* Template Sections */}
          <div className="mb-6">
            <h3 className="text-lg font-semibold text-gray-900 dark:text-slate-100 mb-4">Template Sections</h3>
                ...
                >
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      ...
                      {section.description && (
                        <p className="text-xs text-gray-600 dark:text-slate-400">{section.description}</p>
                      )}
          ...
          {template.export_formats.length > 0 && (
            <div className="mb-6">
              <h3 className="text-lg font-semibold text-gray-900 dark:text-slate-100 mb-4">Supported Export Formats</h3>
              <div className="flex flex-wrap gap-2">
                {template.export_formats.map((format, index) => (
                  <span
                    key={index}
                    className="px-3 py-1 text-sm font-medium text-gray-700 dark:text-slate-300 bg-gray-100 dark:bg-slate-700 rounded-full uppercase"
                  >
                    {format}
                  </span>
                ))}
          ...
          {template.ai_suggestions && Object.keys(template.ai_suggestions).length > 0 && (
            <div className="mb-6">
              <h3 className="text-lg font-semibold text-gray-900 dark:text-slate-100 mb-4">AI Capabilities</h3>
          ...
          {template.branding_config && Object.keys(template.branding_config).length > 0 && (
            <div className="mb-6">
              <h3 className="text-lg font-semibold text-gray-900 dark:text-slate-100 mb-4">Branding</h3>
          ...
          {/* Usage Stats */}
          <div className="grid grid-cols-3 gap-4 p-4 bg-gray-50 dark:bg-slate-800/50 border border-gray-200 dark:border-slate-700 rounded-lg">
            <div className="text-center">
              <div className="text-2xl font-bold text-gray-900 dark:text-slate-100">{template.usage_count}</div>
              <div className="text-xs text-gray-500 dark:text-slate-400">Times Used</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-gray-900 dark:text-slate-100">
                {template.rating?.toFixed(1) || 'N/A'}
              </div>
              <div className="text-xs text-gray-500 dark:text-slate-400">Average Rating</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-gray-900 dark:text-slate-100">{template.sections.length}</div>
              <div className="text-xs text-gray-500 dark:text-slate-400">Sections</div>
            </div>
          </div>
        </div>
