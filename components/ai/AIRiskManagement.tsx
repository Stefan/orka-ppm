...
export default function AIRiskManagement({ 
  ...
  const getUrgencyIcon = (urgency: string) => {
    switch (urgency) {
      case 'immediate': return <AlertTriangle className="h-4 w-4 text-red-600" />
      case 'within_24h': return <Clock className="h-4 w-4 text-orange-600" />
      case 'within_week': return <Eye className="h-4 w-4 text-yellow-600" />
      case 'monitor': return <Info className="h-4 w-4 text-blue-600" />
  ...
  const getPatternTypeIcon = (type: string) => {
    switch (type) {
      ...
      case 'cascading': return <TrendingUp className="h-4 w-4" />
      case 'seasonal': return <Clock className="h-4 w-4" />
      case 'project_phase': return <Target className="h-4 w-4" />
      case 'resource_dependent': return <Shield className="h-4 w-4" />
    ...
    return (
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <div className="flex items-center justify-center space-x-3">
          ...
          <span className="text-gray-600">Analyzing risks with AI...</span>
        </div>
      </div>
    )
  }
      ...
      {dashboardData && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-700">Active Patterns</p>
          ...
          <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-700">Escalation Alerts</p>
          ...
          <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-700">Critical Risks</p>
          ...
          <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-700">Prediction Confidence</p>
                    ...
                    <div key={pattern.pattern_id} className="border border-gray-200 rounded-lg p-4">
                      <div className="flex items-start justify-between mb-3">
                        ...
                        <div className="flex items-center space-x-2">
                          <span className={`px-2 py-1 text-xs rounded-full ${
                            pattern.confidence_score > 0.8 ? 'bg-green-100 text-green-800' :
                            pattern.confidence_score > 0.6 ? 'bg-yellow-100 text-yellow-800' :
                      ...
                      <div className="grid grid-cols-2 gap-4 text-sm">
                        <div>
                          ...
                          <span className="ml-1 capitalize">{pattern.frequency}</span>
                        </div>
                        <div>
                          <span className="font-medium text-gray-700">Occurrences:</span>
                          <span className="ml-1">{pattern.occurrences_count}</span>
                        </div>
                        <div>
                          <span className="font-medium text-gray-700">Avg Impact:</span>
                          <span className="ml-1">{(pattern.average_impact_score * 100).toFixed(0)}%</span>
                        </div>
                        <div>
                          <span className="font-medium text-gray-700">Escalation Risk:</span>
                          <span className="ml-1">{(pattern.escalation_probability * 100).toFixed(0)}%</span>
                        </div>
                      </div>
                      ...
                      {pattern.next_likely_occurrence && (
                        <div className="mt-3 p-3 bg-blue-50 rounded-md">
                          <div className="flex items-center space-x-2 text-sm">
                            ...
                            <span className="font-medium text-blue-900">Next Occurrence:</span>
                    ...
                    >
                      <div className="flex items-start justify-between mb-3">
                        <div className="flex items-start space-x-3">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-gray-600">{alert.project_name}</p>
                          </div>
                        </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-3 text-sm">
                        <div>
                          ...
                          <span className="ml-1">{(alert.current_risk_score * 100).toFixed(0)}%</span>
                        </div>
                        <div>
                          <span className="font-medium text-gray-700">Predicted Score:</span>
                          <span className="ml-1 text-orange-600">
                            {(alert.predicted_risk_score * 100).toFixed(0)}%
                          </span>
                        </div>
                        <div>
                          <span className="font-medium text-gray-700">Time to Escalation:</span>
                          <span className="ml-1">{alert.time_to_escalation}</span>
                        </div>
                      </div>
              ...
              {!selectedRisk ? (
                ...
                <div className="space-y-4">
                  {strategies.map((strategy) => (
                    <div key={strategy.strategy_id} className="border border-gray-200 rounded-lg p-4">
                      <div className="flex items-start justify-between mb-3">
                        <div>
                        ...
                        <div className="flex items-center space-x-2">
                          <span className={`px-2 py-1 text-xs rounded-full ${
                            strategy.historical_success_rate > 0.8 ? 'bg-green-100 text-green-800' :
                            strategy.historical_success_rate > 0.6 ? 'bg-yellow-100 text-yellow-800' :
                            'bg-gray-100 text-gray-800'
                          }`}>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-3 text-sm">
                        ...
                        <div>
                          ...
                          <span className={`ml-1 capitalize ${
                            strategy.implementation_complexity === 'low' ? 'text-green-600' :
                            strategy.implementation_complexity === 'medium' ? 'text-yellow-600' :
                            'text-red-600'
                          }`}>
                        ...
                        <div>
                          ...
                          <span className="ml-1">{strategy.typical_implementation_time}</span>
                        </div>
                      </div>
                          ...
                          <div className="space-y-2">
                            {strategy.implementation_steps.slice(0, 3).map((step) => (
                              <div key={step.step_number} className="flex items-start space-x-2 text-sm">
                                <span className="flex-shrink-0 w-5 h-5 bg-blue-100 text-blue-800 rounded-full text-xs flex items-center justify-center">
                                  {step.step_number}
                                </span>
                                <div>
              ...
              {dashboardData?.ai_insights && (
                <div className="bg-white border border-gray-200 rounded-lg p-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      <div className="text-sm text-gray-700 mb-2">Overall Accuracy</div>
                      <div className="flex items-center space-x-2">
                    ...
                    <div>
                      <div className="text-sm text-gray-700 mb-2">Prediction Confidence</div>
                      <div className="flex items-center space-x-2">
                        <div className="flex-1 bg-gray-200 rounded-full h-2">
                          <div 
