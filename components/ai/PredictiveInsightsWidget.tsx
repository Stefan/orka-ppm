...
export const PredictiveInsightsWidget: React.FC<PredictiveInsightsWidgetProps> = ({
  ...
  if (error) {
    return (
      <div className={`bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-gray-200 dark:border-slate-700 p-4 ${className}`}>
        ...
        <p className="text-xs text-gray-500 dark:text-slate-400">{error}</p>
      </div>
    )
  }
  ...
  const topInsights = [
    ...criticalAlerts.slice(0, 2).map(alert => ({
      type: 'alert',
      icon: AlertTriangle,
      iconColor: alert.severity === 'critical' ? 'text-red-500 dark:text-red-400' : 'text-orange-500',
    ...
    ...performanceInsights.slice(0, 2).map(insight => ({
      ...
      iconColor: insight.level === 'warning' ? 'text-yellow-500 dark:text-yellow-400' : 'text-blue-500 dark:text-blue-400',
    ...
    <div className={`bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-gray-200 dark:border-slate-700 ${className}`}>
      ...
      <div className="p-4 border-b border-gray-200 dark:border-slate-700">
        <div className="flex items-center justify-between">
          <div className="flex items-center">
            ...
            <h3 className="font-medium text-gray-900 dark:text-slate-100">AI Insights</h3>
          </div>
          <button
      ...
      <div className="p-4">
        <div className="grid grid-cols-2 gap-4 mb-4">
          ...
          <div className="text-center">
            <div className="flex items-center justify-center mb-1">
              ...
              <span className="text-xs text-gray-500 dark:text-slate-400">Predicted</span>
            </div>
            <div className="flex items-center justify-center">
              ...
              {utilizationTrend === 'increasing' && (
                <TrendingUp className="h-3 w-3 text-green-500 dark:text-green-400 ml-1" />
              )}
              {utilizationTrend === 'decreasing' && (
                <TrendingDown className="h-3 w-3 text-red-500 dark:text-red-400 ml-1" />
              )}
            </div>
          </div>
        </div>
        ...
        {topInsights.length > 0 && (
          <div className="space-y-2">
            {topInsights.map((insight, index) => (
              <div key={index} className="flex items-start p-2 bg-gray-50 dark:bg-slate-800/50 rounded-lg">
        ...
        {isExpanded && (
          <div className="mt-4 pt-4 border-t border-gray-200 dark:border-slate-700">
            ...
            {dashboardData.learning_metrics && (
              <div className="mb-4">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-xs font-medium text-gray-600 dark:text-slate-400">Model Accuracy</span>
            ...
            {dashboardData.performance_patterns && (
              <div className="mb-4">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-medium text-gray-600 dark:text-slate-400">Active Patterns</span>
                  <span className="text-xs text-gray-900 dark:text-slate-100">
        ...
        {showFullDashboardLink && (
          <div className="mt-4 pt-4 border-t border-gray-200 dark:border-slate-700">
            <button className="w-full flex items-center justify-center px-3 py-2 text-xs font-medium text-indigo-600 dark:text-indigo-400 bg-indigo-50 rounded-md hover:bg-indigo-100 dark:bg-indigo-900/30 transition-colors">
