  ...
  return (
    <div className="relative">
      <button
        ...
        className="p-2 text-gray-600 hover:text-gray-900 dark:hover:text-slate-100 dark:text-slate-100 hover:bg-gray-100 dark:hover:bg-slate-600 dark:bg-slate-700 rounded-full transition-colors"
        title="Setup Help"
      >
      ...
      {isOpen && (
        <div className="absolute right-0 top-full mt-2 w-96 bg-white dark:bg-slate-800 rounded-lg shadow-lg border border-gray-200 dark:border-slate-700 z-50">
          <div className="p-4">
            ...
            <div className="space-y-4">
              <div className="bg-blue-50 border border-blue-200 dark:border-blue-800 rounded-lg p-3">
                <p className="text-sm text-blue-800 dark:text-blue-300">{helpData.message}</p>
              </div>

              <div>
                <h4 className="font-medium text-gray-900 dark:text-slate-100 mb-2 flex items-center">
                  <Shield className="h-4 w-4 mr-1" />
                  User Management
                </h4>
                <p className="text-sm text-gray-600 dark:text-slate-400 mb-2">{helpData.user_management.access}</p>
                
                {helpData.user_management.create_users && (
                  <p className="text-sm text-gray-600 dark:text-slate-400 mb-2">{helpData.user_management.create_users}</p>
                )}

                {helpData.user_management.roles && (
                  <div className="mb-2">
                    <p className="text-xs font-medium text-gray-700 dark:text-slate-300 mb-1">Available Roles:</p>
                ...
                {helpData.user_management.development_features && (
                  <div className="mb-2">
                    <p className="text-xs font-medium text-gray-700 dark:text-slate-300 mb-1">Development Features:</p>
                ...
                {helpData.user_management.mock_users && (
                  <div className="mb-2">
                    <p className="text-xs font-medium text-gray-700 dark:text-slate-300 mb-1">Mock Users Available:</p>
                    <div className="space-y-1">
                      {helpData.user_management.mock_users.map((user, index) => (
                        <div key={index} className="text-xs text-gray-600 dark:text-slate-400 bg-gray-50 dark:bg-slate-800/50 p-2 rounded">
              ...
              {helpData.development_info && (
                <div className="bg-yellow-50 border border-yellow-200 dark:border-yellow-800 rounded-lg p-3">
                  <p className="text-sm text-yellow-800 dark:text-yellow-300 mb-2">{helpData.development_info.note}</p>
                  <div className="flex items-center space-x-4">
                    <a
                      href="/docs"
                      target="_blank"
              ...
              {helpData.security && (
                <div className="bg-red-50 border border-red-200 dark:border-red-800 rounded-lg p-3">
                  <p className="text-sm text-red-800 dark:text-red-300 mb-1">{helpData.security.note}</p>
              ...
              {helpData.production_note && (
                <div className="bg-gray-50 dark:bg-slate-800/50 border border-gray-200 dark:border-slate-700 rounded-lg p-3">
                  <p className="text-sm text-gray-700 dark:text-slate-300">{helpData.production_note}</p>
                </div>
              )}
            </div>
          </div>
