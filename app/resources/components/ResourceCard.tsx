...
function ResourceCard({ resource, onViewDetails, onEdit, onDelete }: ResourceCardProps) {
  return (
    <div className="bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-gray-200 dark:border-slate-700 p-4 sm:p-6 hover:shadow-md transition-shadow touch-manipulation">
      ...
      <div className="mt-4 pt-4 border-t border-gray-100 dark:border-slate-700 flex justify-between items-center flex-wrap gap-2">
...
const arePropsEqual = (prevProps: ResourceCardProps, nextProps: ResourceCardProps) => {
  ...
  if (prevProps.onViewDetails !== nextProps.onViewDetails || prevProps.onEdit !== nextProps.onEdit || prevProps.onDelete !== nextProps.onDelete) return false
  return (
    prev.id === next.id &&
    prev.name === next.name &&
